...
GeoLMBiEncoderPassage = collections.namedtuple("GeoLMBiEncoderPassage", ["text", "title","start_pos","end_pos","lat","lng"])
...
class GeoLMBiEncoderSample(object):
    query: GeoLMBiEncoderPassage
    positive_passages: List[GeoLMBiEncoderPassage]
    negative_passages: List[GeoLMBiEncoderPassage]
    hard_negative_passages: List[GeoLMBiEncoderPassage]


class GeoLMJsonQADataset(Dataset):
    def __init__(
        self,
        file: str,
        selector: DictConfig = None,
    ...
    def __getitem__(self, index) -> GeoLMBiEncoderSample:
        json_sample = self.data[index]
        r = GeoLMBiEncoderSample()
        r.query = GeoLMBiEncoderPassage(
                self._process_query(json_sample["question"]),
                title = None,
                start_pos = json_sample["start_pos"],
                end_pos =  json_sample["end_pos"],
                lat = json_sample["lat"],
                lng = json_sample["lng"]
            )
